...
const TAB_CONFIG = [
  ...
  { key: "topTracksWeek", label: "Top Tracks Week", type: "track" },
  { key: "topTracksMonth", label: "Top Tracks Month", type: "track" },
  { key: "topTracksAllTime", label: "Top Tracks All Time", type: "track" },
  { key: "topArtistsWeek", label: "Top Artists Week", type: "artist" },
  { key: "topArtistsMonth", label: "Top Artists Month", type: "artist" },
  { key: "topArtistsAllTime", label: "Top Artists All Time", type: "artist" },
  { key: "topAlbumsWeek", label: "Top Albums Week", type: "album" },
  { key: "topAlbumsMonth", label: "Top Albums Month", type: "album" },
  { key: "topAlbumsAllTime", label: "Top Albums All Time", type: "album" },
...
function buildWeeklyRows(stats) {
  const sections = [
    { title: "TRACKS", items: normalizeArray(stats?.weeklyTrackChart), type: "track" },
    { title: "ARTISTS", items: normalizeArray(stats?.weeklyArtistChart), type: "artist" },
    { title: "ALBUMS", items: normalizeArray(stats?.weeklyAlbumChart), type: "album" },
  ];
  ...
  return normalizeArray(stats?.[tabKey]).map((item, index) => {
    ...
    if (tabType === "album") {
      return {
        ...
        highlight:
          ...
          currentAlbum &&
          currentAlbum === (item.album || item.name),
      };
    }
    ...
    if (tabKey === "recentTracks") {
      return {
        ...
        title: `${item.artist || "Unknown"} - ${item.track || "Untitled"}`,
        subtitle: item.album || "",
        meta: item.nowPlaying ? "NOW PLAYING" : formatListenedAt(item.playedAt),
    ...
    return {
      ...
      highlight:
        currentArtist === item.artist &&
        currentTrack &&
        currentTrack === (item.track || item.name),
    };
  });
}
...
function Spotify() {
  ...
  useEffect(() => {
    if (!SpotifyExpand.show || stats || loading) return undefined;
  ...
  }, [SpotifyExpand.show, stats, loading]);

  useEffect(() => {
    if (!SpotifyExpand.show) return undefined;
  ...
  const nowPlaying = stats?.nowPlaying || normalizeArray(stats?.recentTracks)[0] || null;
  const ledText = nowPlaying
    ? `${nowPlaying.artist || "Unknown"} - ${nowPlaying.track || "Nothing Selected"}`
    : "SPOTIFY READY :: CONNECT TO LAST.FM";
  ...
  return (
    <AppWindowShell
      ...
      className="spotify-app-window"
    >
      <div className="spotify-shell">
        <div className="spotify-player-column">
          <div className="spotify-module-head">SPOTIFY PLAYER</div>

          <div className="spotify-led-panel">
            <div className="spotify-led-track">
              <div className="spotify-led-index">1.</div>
              <div className="spotify-led-marquee">
                <span>{ledText}</span>
              </div>
            </div>
            <div className="spotify-led-bottom">
              <div className="spotify-digital-time">{formatElapsed(elapsed)}</div>
              <div className="spotify-led-mini-grid">
          ...
          <div className="spotify-status-strip">
            <span>128 kbps</span>
            <span>44 kHz</span>
            <span>stereo</span>
          </div>
          ...
          <div className="spotify-controls">
            <div className="spotify-button-row">
              {PLAYER_BUTTONS.map((button) => (
                <button key={button.key} type="button" className="spotify-control-button" title={button.title}>
                  {button.label}
                </button>
              ))}
            </div>
            <div className="spotify-toggle-row">
              <button
                type="button"
                className={`spotify-mode-button ${shuffleEnabled ? "active" : ""}`}
              ...
              <button
                type="button"
                className={`spotify-mode-button ${repeatEnabled ? "active" : ""}`}
          ...
          <div className="spotify-slider-group">
            <label>
              <span>VOL</span>
              <input
                type="range"
          ...
          <div className="spotify-spectrum-panel">
            {visualizerBars.map((value, index) => (
              <span key={`spectrum-${index}`} style={{ height: `${18 + value / 1.8}px` }} />
            ))}
          </div>

          <div className="spotify-user-panel">
            <div className="spotify-user-row">
        ...
        <div className="spotify-eq-column">
          <div className="spotify-module-head">SPOTIFY EQUALIZER</div>
          <div className="spotify-eq-toolbar">
            <button type="button" className="spotify-eq-button active">ON</button>
            <button type="button" className="spotify-eq-button">AUTO</button>
            <button type="button" className="spotify-eq-button">PRESETS</button>
          </div>

          <div className="spotify-eq-panel">
            <div className="spotify-eq-scale">
              <span>+12db</span>
              <span>0db</span>
              <span>-12db</span>
            </div>
            <div className="spotify-eq-sliders">
              {EQ_BANDS.map((band, index) => (
                <label key={band} className="spotify-eq-band">
                  <input
                    type="range"
                    min="-12"
                    max="12"
        ...
        <div className="spotify-playlist-column">
          <div className="spotify-module-head">SPOTIFY PLAYLIST</div>
          ...
          <div className="spotify-tabs">
            {TAB_CONFIG.map((tab) => (
              <button
                key={tab.key}
                type="button"
                className={`spotify-tab ${activeTab === tab.key ? "active" : ""}`}
          ...
          <div className="spotify-playlist-content">
            {loading ? (
              ...
              <div className="spotify-status-card">
                <p>{error}</p>
                <button type="button" className="spotify-small-button" onClick={handleRetry}>
                  RETRY
                </button>
              </div>
            ) : (
              <div className="spotify-playlist-list">
                {playlistRows.length === 0 ? (
                  ...
                  playlistRows.map((row) =>
                    row.kind === "section" ? (
                      <div key={row.key} className="spotify-playlist-section">
                        {row.title}
                      </div>
                    ) : (
                      <div
                        key={row.key}
                        className={`spotify-playlist-row ${row.highlight ? "active" : ""}`}
                      >
                        <span className="spotify-playlist-index">{row.index}</span>
                        <div className="spotify-playlist-copy">
                          <span className="spotify-playlist-title">{row.title}</span>
                          {row.subtitle ? (
                            <span className="spotify-playlist-subtitle">{row.subtitle}</span>
                          ) : null}
                        </div>
                        <span className="spotify-playlist-meta">{row.meta}</span>
                      </div>
                    ),
                  )
                )}
          ...
          <div className="spotify-playlist-footer">
            <div className="spotify-footer-buttons">
              <button type="button" className="spotify-small-button">ADD</button>
              <button type="button" className="spotify-small-button">REM</button>
              <button type="button" className="spotify-small-button">SEL</button>
              <button type="button" className="spotify-small-button">MISC</button>
            </div>
            <div className="spotify-footer-stats">
